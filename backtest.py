...
# Your exchange: 0.070% maker / 0.070% taker per side (one-way)
# Using taker on both sides = 0.070% * 2 = 0.140%
FEES_ROUNDTRIP = 0.00140
# Slippage per side for market orders on BTC/USDT: ~0.05%
...
async def fetch_all_candles(session):
    all_candles = []
    cursor = START
    window = timedelta(days=180)  # 6-month windows, ~1095 bars each, well under 2000 limit
...
def compute_stats(trades, start, end, closes):
    ...
    print("\n" + "=" * 50)
    print("PHASE 0.5 — ECONOMICS CHECK")
    print("=" * 50)
